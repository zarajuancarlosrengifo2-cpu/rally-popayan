...
const formatMillions = (num: number) =>
  `$${((Number(num) || 0) / 1000000).toFixed(0)}M`;

const calculateScore = (kpis: any, status: string) => {
  if (!kpis) return 0;
  let score = 0;

  const contencionRatio =
    METAS.contencion > 0 ? Math.min(kpis.contencion / METAS.contencion, 1) : 0;
  score += contencionRatio * 300;

  const totalColocadas = kpis.colocacion.nuevas + kpis.colocacion.renovaciones;
  const colocacionRatio =
    METAS.colocacion_total > 0
      ? Math.min(totalColocadas / METAS.colocacion_total, 1)
      : 0;
  const nuevasRatio =
  ...
  const pagaRatio =
    METAS.pagahorros > 0 ? Math.min(kpis.pagahorros / METAS.pagahorros, 1) : 0;
  score += pagaRatio * 150;
...
const getUsersWithScores = (users: any[]) =>
  users.map((u) => {
    ...
    return {
      ...safeUser,
      score: calculateScore(safeUser.kpis, safeUser.status),
    };
  });
...
const Kart3D = memo(
  ({
    ...
    const auraClass = isActive
      ? `bg-gradient-to-br ${vData.bg} opacity-50 blur-lg group-hover:opacity-100`
      : isPits
      ? 'bg-yellow-400 opacity-30 blur-lg'
      : 'bg-slate-400 opacity-10 blur-sm';

    const pedestalClass = isActive
      ? `bg-white/60 border-white/90 border-t border-b-4 border-black/10 ${league.border} border-2`
      : isPits
      ? 'bg-yellow-100/60 border-yellow-400/50 border-t border-b-4 border-yellow-700/20 border-2'
      : 'bg-slate-200/50 border-slate-300/50 border-t border-b-2 border-slate-400/20';

    const emojiAnimClass = isActive
      ? 'drop-shadow-md transform -translate-y-2'
      : isPits
      ? 'sepia-[.4] transform translate-y-0'
      : 'grayscale opacity-70 transform translate-y-0';
    ...
    return (
      ...
      >
        ...
        />
        <div
          className={`absolute inset-1 backdrop-blur-sm rounded-full shadow-sm transition-colors duration-500 ${pedestalClass}`}
        />
        <span
          className={`relative z-10 ${emojiSize} transition-all duration-500 ${emojiAnimClass} select-none`}
        >
          {vData.emoji}
        </span>
      </div>
...
const MetricCard = memo(({ title, value, meta, type = 'normal' }: any) => {
  ...
  return (
    <div className="w-full">
      <span className="opacity-70 block text-[9px] uppercase tracking-wider mb-0.5 flex items-center gap-1">
        {title}
      </span>
      <div className="flex items-end justify-between mb-1">
        <span className="text-lg leading-none font-black text-white">
          {type === 'money' ? formatMillions(valNum) : valNum}
          {type === 'percent' ? '%' : ''}
        </span>
        <span className="opacity-60 text-[9px] font-normal">
          Meta: {type === 'money' ? formatMillions(metaNum) : metaNum}
          {type === 'percent' ? '%' : ''}
        </span>
      </div>
...
const PitsTopUserButton = memo(({ u, isSelected, onSelect }: any) => (
  <button
    ...
    className={`snap-center flex-shrink-0 relative w-20 h-24 rounded-[2rem] transition-all duration-300 pt-4 ${
      isSelected
        ? 'bg-[#ED1C24] shadow-[0_10px_30px_rgba(237,28,36,0.3)] scale-110 border-none z-10'
        : 'bg-white/70 border border-white/60 shadow-sm hover:bg-white/90'
    }`}
  >
    <div className="absolute -top-6 left-1/2 -translate-x-1/2 relative">
      <Kart3D
        ...
        className={`w-16 h-16 transition-all duration-300 ${
          !isSelected ? 'opacity-80 scale-75' : ''
        }`}
        emojiSize="text-3xl"
...
const SplashScreen = ({ onComplete }: any) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-900 text-white transition-opacity duration-500 ease-in-out ${
    ...
    >
      ...
      <div className="flex flex-col items-center flex-1 justify-center w-full max-w-md px-8 relative z-10">
        ...
        <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden mb-6 relative border border-slate-700">
          <div
            className="h-full bg-gradient-to-r from-red-600 to-orange-500 rounded-full transition-all duration-100 ease-out"
...
const Dashboard = ({ users, openManagerPanel }: any) => {
  const {
    ...
    promContencion,
    totalColocadas,
    totalRodamientosRecuperados,
    totalRodamientosBase,
    totalPagahorros,
    pilotosAutorizados,
    pilotosTotal,
    autorizadosRatio,
    totalFaltaRecuperar,
    progresoRecuperacionGlobal,
    preventivasOk,
    agendasOk,
  } = useMemo(() => {
    const tUser =
    ...
    const pCont =
      users.reduce(
        (acc: number, u: any) => acc + (u.kpis?.contencion || 0),
        0
      ) / validUsers;
    const tCol = users.reduce(
      (acc: number, u: any) =>
        acc +
    ...
    const tRec = users.reduce(
      (acc: number, u: any) => acc + (u.kpis?.rodamientos?.recuperado || 0),
      0
    );
    const tBas = users.reduce(
      (acc: number, u: any) => acc + (u.kpis?.rodamientos?.base || 0),
      0
    );
    const tPag = users.reduce(
      (acc: number, u: any) => acc + (u.kpis?.pagahorros || 0),
      0
    );
    ...
    const prevOkCount = users.filter(
      (u: any) => u.preventiva?.realizada
    ).length;
    ...
    return {
      ...
      topLeague: tLeague,
      badges: bdgs,
      promContencion: pCont,
      totalColocadas: tCol,
      totalRodamientosRecuperados: tRec,
      totalRodamientosBase: tBas,
      totalPagahorros: tPag,
      pilotosAutorizados: pAut,
      pilotosTotal: pTot,
      autorizadosRatio: autRat,
      totalFaltaRecuperar: tFalta,
      progresoRecuperacionGlobal: gProg,
      preventivasOk: prevOkCount,
      agendasOk: agendaOkCount,
    };
  }, [users]);
  ...
  return (
    <div className="pb-32 pt-8 px-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-slate-500 font-bold text-sm uppercase tracking-wider">
            Pole Position
          </h2>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            <h3 className="font-black text-sm uppercase tracking-widest text-red-400">
              Dirección Deportiva
            </h3>
            <p className="text-[10px] font-bold text-slate-400 leading-tight">
              Control de Pista y Escudería
            </p>
          </div>
        </div>
      ...
      {topUser ? (
        <div className="relative bg-gradient-to-br from-[#ED1C24] to-[#B0151B] rounded-[3rem] p-7 shadow-[0_20px_40px_rgba(237,28,36,0.3)] border border-red-400/30 overflow-visible mt-12 group">
          ...
          <div className="relative z-10 w-full">
            <div className="flex items-center space-x-2 mb-3">
              ...
              <div
                className={`inline-flex items-center space-x-1 bg-gradient-to-r ${
                  topLeague?.color || 'from-slate-400 to-slate-500'
                } rounded-full px-3 py-1.5 shadow-sm border border-white/40`}
              >
                <span className="text-white text-[10px] font-black uppercase tracking-wider">
                  Liga {topLeague?.name || 'Base'}
                </span>
              </div>
            </div>

            <h3 className="text-white text-4xl font-black mb-1 leading-tight drop-shadow-sm">
              {topUser.name}
            </h3>
            ...
            {badges.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {badges.map((b) => (
                  <div
                    key={b.id}
                    className="bg-black/20 backdrop-blur-sm border border-white/20 rounded-xl px-2 py-1 flex items-center space-x-1.5 shadow-sm"
                    title={b.desc}
                  >
            ...
            <div className="mb-4 bg-black/20 rounded-2xl p-3 border border-white/10">
              <span className="text-[10px] text-white/80 font-bold uppercase tracking-wider mb-2 block text-center">
              ...
              <div className="flex justify-around items-center">
                ...
                <div className="flex items-center gap-1.5">
                  <BookOpen
                    size={14}
                    className={
                      topUser.preventiva?.agenda
                        ? 'text-green-400'
                        : 'text-slate-400'
                    }
                  />
                  <span className="text-white font-black text-xs">
                    {topUser.preventiva?.agenda ? '✅ Agenda' : '❌ Agenda'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-3 text-white font-bold bg-black/10 rounded-3xl p-4 backdrop-blur-sm border border-white/10">
              ...
              <MetricCard
                title="Rodamientos"
                value={topUser.kpis?.rodamientos?.recuperado}
                meta={topUser.kpis?.rodamientos?.base}
                type="money"
              />

              <div className="col-span-2 border-t border-white/10 pt-3">
                <span className="opacity-70 block text-[9px] uppercase tracking-wider mb-1">
                  Colocación (Meta Total: {METAS.colocacion_total})
                </span>
                <div className="flex justify-between items-center bg-white/10 rounded-xl px-3 py-2 border border-white/5">
                  <div>
                    ...
                    <span className="text-white/60 font-bold text-[9px] ml-1">
        ...
        <div className="bg-white/50 backdrop-blur-xl rounded-[2rem] p-8 text-center border border-white/80 mt-12 shadow-sm">
          ...
          <p className="text-xs text-slate-500 mt-2">
            Usa el panel de Dirección Deportiva para agregar analistas a la
            escudería.
          </p>
        </div>
      )}

      <h3 className="text-slate-500 font-bold text-xs uppercase tracking-wider mt-8 mb-2 pl-2">
        Marcadores de Oficina
      </h3>
      ...
      <div className="bg-white/90 backdrop-blur-md rounded-[2.5rem] p-5 border border-white shadow-sm flex flex-col justify-between mb-4">
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center space-x-2">
            ...
            <p className="text-slate-600 font-black text-sm uppercase tracking-wider">
              Escudería Autorizada
            </p>
          </div>
          <span className="text-2xl font-black text-slate-800">
            {pilotosAutorizados}/{pilotosTotal}
          </span>
        </div>
        <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden shadow-inner">
          <div
            className={`h-full rounded-full transition-all duration-700 ${
              autorizadosRatio >= 100
        ...
        <div className="flex justify-between mt-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
          <span className="flex items-center gap-1">
            <PhoneCall size={12} /> {preventivasOk} Preventivas
          </span>
          <span className="flex items-center gap-1">
            <BookOpen size={12} /> {agendasOk} Agendas
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white/70 backdrop-blur-md rounded-[2.5rem] p-5 border border-white/80 shadow-sm flex flex-col justify-between group hover:shadow-md transition-all cursor-default">
          ...
          <p className="text-xl font-black text-slate-800 tracking-tight">
            {formatMillions(totalRodamientosRecuperados)}
          </p>
        </div>
        <div className="bg-white/70 backdrop-blur-md rounded-[2.5rem] p-5 border border-white/80 shadow-sm flex flex-col justify-between group hover:shadow-md transition-all cursor-default">
      ...
      <div className="mt-4 relative bg-white/70 backdrop-blur-md rounded-[2.5rem] border-2 border-orange-400/50 shadow-sm overflow-hidden group">
        ...
        <div className="relative p-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="bg-white px-3 py-1 rounded-full text-[9px] font-bold text-slate-500 border border-slate-100">
              Meta Oficial
            </span>
          </div>
          ...
          <div className="w-full">
            <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider mb-1.5">
              ...
              <span className="text-orange-600 font-black">
                {Math.round(progresoRecuperacionGlobal)}%
              </span>
            </div>
            <div className="w-full bg-orange-100/60 rounded-full h-2.5 overflow-hidden shadow-inner border border-orange-200/50">
              <div
                ...
                style={{
                  width: `${Math.min(progresoRecuperacionGlobal, 100)}%`,
                }}
              />
            </div>
          </div>
...
const Track = memo(({ users }: any) => {
  if (users.length === 0)
    return (
      <div className="p-8 text-center text-slate-500 font-bold mt-12">
        Aún no hay pilotos en la pista.
      </div>
    );
  ...
  const { milestones, currentDay, monthName, timeProgress } = useMemo(() => {
    ...
    const mName = new Intl.DateTimeFormat('es-CO', { month: 'long' }).format(
      today
    );
    ...
    const ms = [
      {
        label: '🏁 Cierre de Mes (Meta)',
        date: `${daysInMonth} de ${mName}`,
        threshold: 80000,
        users: [],
      },
      {
        label: '📍 Corte 3',
        date: `${Math.floor(daysInMonth * 0.75)} de ${mName}`,
        threshold: 50000,
        users: [],
      },
      {
        label: '📍 Corte 2',
        date: `${Math.floor(daysInMonth * 0.5)} de ${mName}`,
        threshold: 25000,
        users: [],
      },
  ...
  return (
    <div className="pb-32 pt-8 px-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h2 className="text-slate-500 font-bold text-sm uppercase tracking-wider mb-1">
            Avance del Mes
          </h2>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight italic capitalize">
            Circuito {monthName}
          </h1>
        </div>
        <div className="text-right">
          <span className="text-3xl font-black text-[#ED1C24]">
            {currentDay}
          </span>
          ...
          {milestones.map((ms: any, idx: number) => (
            <div key={idx} className="relative pl-24 min-h-[90px]">
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-6 mt-2">
                {ms.users.map((u: any) => (
                  ...
                  >
                    ...
                    <div className="mt-3 text-center">
                      <p className="text-[11px] font-black text-slate-700 leading-tight mb-1">
                        {u.name}
                      </p>
                ...
                {ms.users.length === 0 && (
                  <p className="text-slate-400 text-xs font-bold italic w-full ml-4">
                    Tramo despejado
                  </p>
                )}
              </div>
...
const Pits = ({ users, updateUserKpis }: any) => {
  ...
  useEffect(() => {
    if (
      users.length > 0 &&
      (!selectedUserId || !users.find((u: any) => u.id === selectedUserId))
    ) {
  ...
  if (users.length === 0)
    return (
      <div className="p-8 text-center text-slate-500 font-bold mt-12">
        Agrega pilotos desde Dirección Deportiva para ajustar la telemetría.
      </div>
    );
  ...
  const handleInputChange = useCallback(
    (category: string, field: string, subfield: string | null, value: any) => {
    ...
    <div className="pb-40 pt-8 px-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col relative">
      {showPinModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 sm:p-6 animate-in fade-in duration-300">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl w-full max-w-sm flex flex-col items-center animate-in zoom-in-95 duration-300">
            ...
            <h3 className="font-black text-slate-800 text-xl mb-1 text-center">
              Acceso Dirección
            </h3>
            <p className="text-xs text-slate-500 font-bold mb-6 text-center">
              Ingresa el PIN para actualizar la telemetría.
            </p>
            <input
              ref={pinInputRef}
              type="password"
              ...
              className={`w-full bg-slate-100 rounded-2xl px-4 py-4 text-center font-black text-3xl tracking-[0.5em] mb-2 outline-none border-2 transition-colors ${
                pinError
                  ? 'border-red-400 bg-red-50 text-red-600'
                  : 'border-transparent focus:border-[#ED1C24] text-slate-800'
              }`}
              placeholder="••••"
            />
            <div className="h-6">
              {pinError && (
                <p className="text-red-500 text-[11px] font-black uppercase tracking-wider text-center animate-bounce">
                  PIN Incorrecto
                </p>
              )}
            </div>
            <div className="flex gap-3 mt-2 w-full">
              <button
                ...
                className="flex-1 py-3.5 bg-slate-100 text-slate-600 font-black rounded-2xl active:scale-95 transition-transform"
              >
                Cancelar
              </button>
              <button
                onClick={submitPin}
                className="flex-1 py-3.5 bg-[#ED1C24] text-white font-black rounded-2xl active:scale-95 transition-transform shadow-sm"
              >
                Verificar
              </button>
            </div>
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-800 tracking-tight leading-none italic">
            Pits
          </h1>
        </div>
        {isUnlocked ? (
          <button
            onClick={handleLock}
            className="flex items-center space-x-1 bg-slate-200 text-slate-600 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider hover:bg-slate-300 transition-colors"
          >
            <Lock size={12} />
            <span>Bloquear</span>
          </button>
          ...
          <button
            ...
            className="flex items-center space-x-1 bg-[#ED1C24]/10 text-[#ED1C24] px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider hover:bg-[#ED1C24]/20 transition-colors"
          >
            <Unlock size={12} />
            <span>Editar</span>
          </button>
      ...
      <div className="mt-2 flex-1 relative">
        <div
          className={`w-full bg-white/80 backdrop-blur-md rounded-[3rem] p-6 border border-white/80 shadow-sm transition-all duration-300 ${
            isUpdating ? 'ring-4 ring-green-400 scale-[1.02]' : ''
          }`}
        >
          {!isUnlocked && (
            <div className="absolute inset-0 z-20 backdrop-blur-sm bg-white/60 rounded-[3rem] flex flex-col items-center justify-center p-6 transition-all">
              <div className="bg-white p-8 rounded-[2rem] shadow-lg flex flex-col items-center text-center max-w-[260px] border border-slate-100 transform transition-all hover:scale-105">
                ...
                <p className="text-xs text-slate-500 font-bold mb-6">
                  El ingreso de métricas está protegido.
                </p>
                <button
                  onClick={handleUnlockClick}
                  className="bg-slate-800 text-white px-6 py-3 rounded-2xl font-black text-sm w-full shadow-sm active:scale-95 transition-transform flex items-center justify-center gap-2"
                >
                  <Unlock size={16} />
            ...
            <div className="flex-1">
              ...
              <p
                className={`text-[10px] font-bold uppercase tracking-widest ${
                  showAsSaved ? 'text-green-500' : 'text-slate-400'
                }`}
              >
                {isUnlocked
                  ? showAsSaved
          ...
          >
            ...
            <div className="flex flex-col gap-4 bg-orange-50/50 p-4 rounded-[1.5rem] border border-orange-100">
              <span className="block text-[11px] font-black text-orange-700 uppercase tracking-wider">
                Requisitos de Arranque
              </span>

              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
                  <PhoneCall size={14} className="text-orange-500" /> Preventiva
                  Realizada
                </span>
                <button
                  onClick={() =>
                ...
                >
                  <div
                    className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform shadow-sm ${
                      formData.preventiva?.realizada ? 'left-6' : 'left-1'
                    }`}
                  />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-orange-200/50">
                <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
                  <BookOpen size={14} className="text-orange-500" /> Agenda
                  Presentada
                </span>
                <button
                  onClick={() =>
                ...
                >
                  <div
                    className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform shadow-sm ${
                      formData.preventiva?.agenda ? 'left-6' : 'left-1'
                    }`}
                  />
                </button>
              </div>
            ...
            <div>
              ...
              <div
                className={`flex items-center rounded-2xl border overflow-hidden bg-slate-50 border-slate-200 focus-within:border-slate-400 transition-colors`}
              >
                <input
                  type="text"
                  inputMode="numeric"
                  value={
                    formData.kpis?.contencion === 0
                      ? ''
                      : formData.kpis?.contencion
                  }
                  onChange={(e) =>
                    handleInputChange(
                      'kpis',
                  ...
                  placeholder="0"
                  className="flex-1 bg-transparent px-4 py-3 font-black text-slate-800 outline-none w-full"
                />
                <span className="px-4 font-black text-slate-400 bg-black/5">
                  %
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                  Nuevas{' '}
                  <span className="text-[#ED1C24] text-[9px]">
                    (M: {METAS.colocacion_nuevas})
                  </span>
                </label>
                <input
                  type="text"
                  inputMode="numeric"
                  value={
                    formData.kpis?.colocacion?.nuevas === 0
                      ? ''
                      : formData.kpis?.colocacion?.nuevas
                  }
                  onChange={(e) =>
                    handleInputChange(
                      'kpis',
                  ...
                  placeholder="0"
                  className="w-full bg-slate-50 rounded-2xl px-4 py-3 font-black text-slate-800 border border-slate-200 outline-none focus:border-slate-400 transition-colors"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={
                    formData.kpis?.colocacion?.renovaciones === 0
                      ? ''
                      : formData.kpis?.colocacion?.renovaciones
                  }
                  onChange={(e) =>
                    handleInputChange(
                      'kpis',
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                Rodamientos (Base Asignada)
              </label>
              <div
                className={`flex items-center rounded-2xl border overflow-hidden mb-3 bg-slate-50 border-slate-200 focus-within:border-slate-400 transition-colors`}
              >
                <span className="px-4 font-black text-slate-400 bg-black/5">
                  $
                </span>
              ...
              <div
                className={`flex items-center rounded-2xl border overflow-hidden bg-emerald-50 border-emerald-200 focus-within:border-emerald-400 transition-colors`}
              >
                <span className="px-4 font-black text-emerald-600 bg-emerald-100/50">
                  $
                </span>
                <FormattedNumberInput
                  ...
                  disabled={!isUnlocked}
                  className="flex-1 bg-transparent px-4 py-3 font-black text-emerald-800 outline-none w-full"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={
                    formData.kpis?.pagahorros === 0
                      ? ''
                      : formData.kpis?.pagahorros
                  }
                  onChange={(e) =>
                    handleInputChange(
                      'kpis',
              ...
              <div>
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={
                    formData.kpis?.cuentas_ahorro === 0
                      ? ''
                      : formData.kpis?.cuentas_ahorro
                  }
                  onChange={(e) =>
                    handleInputChange(
                      'kpis',
          ...
          <button
            disabled={!isUnlocked}
            onClick={handleSave}
            className={`w-full mt-8 text-white rounded-2xl py-4 font-black flex items-center justify-center space-x-2 transition-all duration-300 ${
              !isUnlocked ? 'opacity-0 pointer-events-none' : 'opacity-100'
            } ${
              showAsSaved
                ? 'bg-green-500 hover:bg-green-600 shadow-md'
                : 'bg-[#ED1C24] hover:bg-red-700 shadow-md active:scale-95'
            }`}
          >
            {showAsSaved ? <CheckCircle size={20} /> : <Save size={20} />}
            <span>
              {showAsSaved ? 'Telemetría Actualizada' : 'Guardar Telemetría'}
            </span>
          </button>
        </div>
      </div>
...
const Ranking = memo(({ users }: any) => {
  if (users.length === 0)
    return <div className="p-8 text-center">No hay pilotos.</div>;
  const sorted = [...users].sort((a: any, b: any) => b.score - a.score);
  ...
  return (
    <div className="pb-32 pt-8 px-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <h1 className="text-3xl font-black text-slate-800 tracking-tight mb-20 italic">
        Podio de Honor
      </h1>
      ...
      <div className="flex items-end justify-center h-64 mb-16 space-x-2">
        {podiumUsers.map((u, i) => {
          ...
          return (
            <div
              key={u.id}
              className={`relative w-24 ${height} ${bg} rounded-t-[2rem] shadow-sm flex flex-col items-center justify-end pb-4 border-t-4 border-white/80 cursor-default`}
            >
              {/* ETIQUETAS EN EL PODIO */}
              {u.status === 'pits' && (
                <div className="absolute -top-3 -right-3 bg-yellow-400 text-yellow-900 text-[8px] font-black px-2 py-0.5 rounded-full z-30 shadow-sm uppercase border border-white">
                  Bandera Amarilla
                </div>
              )}
              {u.status === 'penalty' && (
                <div className="absolute -top-3 -right-3 bg-red-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full z-30 shadow-sm uppercase border border-white">
                  Penalizado
                </div>
              )}
              ...
              <div className="absolute bottom-full mb-4 w-full flex justify-center group-hover:-translate-y-2 transition-transform duration-300">
                <Kart3D
                  user={u}
                  className={isGold ? 'w-28 h-28' : 'w-20 h-20'}
                  emojiSize={isGold ? 'text-[4.5rem]' : 'text-4xl'}
                  isFloating={false}
                />
              </div>
              <span className="text-slate-800/20 font-black text-6xl absolute top-4 select-none">
                {rank}
              </span>
              <span className="text-slate-800 font-black text-sm z-10 truncate w-full text-center px-1 tracking-tight">
                {u.name}
              </span>
            </div>
          );
      ...
      <div className="space-y-3">
        {rest.map((u, i) => {
          ...
          return (
            <div
              key={u.id}
              className={`bg-white/70 backdrop-blur-sm rounded-[2rem] p-4 flex items-center justify-between shadow-sm transition-all group ${
                isPenalty
                  ? 'border-red-200 opacity-80 bg-red-50/30'
                  : 'border border-white/80'
              }`}
            >
              <div className="flex items-center space-x-4">
                <span className="text-slate-400 font-black w-6 text-center text-sm">
                  {i + 4}
                </span>
                <Kart3D
                  user={u}
                ...
                <div className="flex flex-col">
                  <span className="font-black text-slate-700 group-hover:text-[#ED1C24] transition-colors leading-tight">
                    {u.name}
                  </span>
                  <span
                    className={`text-[9px] font-bold ${league.text} uppercase tracking-wider`}
                  >
                    Liga {league.name}
                  </span>
                </div>
              ...
              <div className="flex flex-col items-end gap-1">
                <span
                  className={`font-black tabular-nums bg-white/60 border px-3 py-1 rounded-full shadow-sm text-sm ${
                    isPenalty
                      ? 'text-red-400 border-red-200'
                      : 'text-[#ED1C24] border-white/50'
                  }`}
                >
                  {formatNumber(u.score)} pts
                </span>
                {u.status === 'pits' && (
                  <span className="text-[8px] bg-yellow-100 text-yellow-700 border border-yellow-300 px-2 py-0.5 rounded-full font-black uppercase shadow-sm">
                    Bandera Amarilla
                  </span>
                )}
                {u.status === 'penalty' && (
                  <span className="text-[8px] bg-red-100 text-red-600 border border-red-300 px-2 py-0.5 rounded-full font-black uppercase shadow-sm">
                    Penalizado (-15k)
                  </span>
                )}
              </div>
...
const Garage = memo(({ users }: any) => {
  ...
  return (
    <div className="pb-32 pt-8 px-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <h1 className="text-3xl font-black text-slate-800 tracking-tight mb-8 italic">
        Garage
      </h1>
      ...
      <div className="space-y-4">
        {users.map((u: any) => {
          ...
          return (
            <div
              key={u.id}
              className={`bg-white/90 backdrop-blur-sm rounded-[2.5rem] p-5 border shadow-sm transition-shadow ${
                u.status === 'active'
                  ? 'border-white/80'
                  : u.status === 'pits'
            ...
            >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-0.5">
                      <span
                        className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-gradient-to-r ${league.color} text-white`}
                      >
                        Liga {league.name}
                      </span>
                ...
                <div className="bg-red-100 text-red-600 rounded-xl p-3 flex flex-col items-center justify-center space-y-1 mb-4 border border-red-200 text-center">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider">
                      Penalizado (-15.000 pts)
                    </span>
                  </div>
                  <span className="text-[9px] font-bold opacity-80">
                    Por no cumplir requisitos de arranque.
                  </span>
                </div>
              )}
              {u.status === 'pits' && (
              ...
              {badges.length > 0 && u.status === 'active' && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {badges.map((b) => (
                    <div
                      key={b.id}
                      className={`${b.bgClass} border rounded-lg px-2 py-1.5 flex items-center space-x-1.5 shadow-sm`}
                      title={b.desc}
                    >
                      <b.Icon size={14} className={b.colorClass} />
                      <span
                        className={`text-[9px] font-black uppercase tracking-wider ${b.colorClass}`}
                      >
                        {b.name}
                      </span>
                    </div>
              ...
              <div
                className={`space-y-4 ${
                  u.status !== 'active' ? 'opacity-60 grayscale' : ''
                }`}
              >
                ...
                <div className="bg-orange-50/50 p-3 rounded-2xl border border-orange-100 flex flex-col gap-2">
                  <div className="flex justify-between items-center pb-2 border-b border-orange-200/50">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-orange-800 flex items-center gap-1.5">
                      <PhoneCall size={12} /> Preventiva Realizada
                    </span>
                    <span
                      className={
                        u.preventiva?.realizada
                          ? 'text-green-600 font-black text-[10px]'
                          : 'text-red-500 font-black text-[10px]'
                      }
                    >
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-orange-800 flex items-center gap-1.5">
                      <BookOpen size={12} /> Agenda Presentada
                    </span>
                    <span
                      className={
                        u.preventiva?.agenda
                  ...
                  <div className="space-y-2">
                    <div>
                      <div className="flex justify-between items-end mb-1">
                        <span className="font-black text-emerald-600 text-sm leading-none">
                          {u.kpis?.colocacion?.nuevas || 0}{' '}
                ...
                <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100/50 shadow-sm">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {faltaRodamientos > 0 ? (
                      <span className="text-[9px] font-bold text-orange-500 flex items-center">
                        <AlertCircle size={10} className="mr-1" /> Faltan $
                        {formatMillions(faltaRodamientos)}
                      </span>
                    ) : (
                  ...
                  <div className="flex justify-between text-xs font-black">
                    <span className="text-emerald-600">
                      {formatMillions(u.kpis?.rodamientos?.recuperado)}
                    </span>
                    <span className="text-slate-300">/</span>
                    <span className="text-slate-600">
                      {formatMillions(u.kpis?.rodamientos?.base)}
                    </span>
                  </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 sm:p-6 animate-in fade-in duration-300">
      <div className="bg-white p-5 sm:p-6 rounded-[2.5rem] shadow-2xl w-full max-w-sm flex flex-col animate-in zoom-in-95 duration-300 relative overflow-hidden max-h-[90vh] sm:max-h-[85vh]">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 p-2 z-10 bg-white rounded-full shadow-sm"
        >
          <X size={20} />
        </button>
        ...
        {!isUnlocked ? (
          <div className="flex flex-col items-center w-full py-4 overflow-y-auto hide-scrollbar">
            ...
            <h3 className="font-black text-slate-800 text-xl mb-1 text-center shrink-0">
              Acceso a Dirección
            </h3>
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && submitPin()}
              className={`w-full bg-slate-100 rounded-2xl px-4 py-4 text-center font-black text-3xl tracking-[0.5em] mb-2 outline-none border-2 transition-colors shrink-0 ${
                pinError
                  ? 'border-red-400 bg-red-50 text-red-600'
                  : 'border-transparent focus:border-[#ED1C24] text-slate-800'
              }`}
            ...
            <button
              onClick={submitPin}
              className="w-full mt-2 py-4 bg-[#ED1C24] text-white font-black rounded-2xl active:scale-95 transition-transform shadow-[0_10px_20px_rgba(237,28,36,0.3)] shrink-0"
            >
              Verificar
            </button>
          </div>
          ...
          <div className="w-full flex flex-col pt-2 flex-1 min-h-0 overflow-hidden">
            <h3 className="font-black text-slate-800 text-lg mb-4 text-center italic tracking-tight shrink-0">
              Panel de Dirección
            </h3>
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl w-full mb-4 shrink-0">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${
                  tab === 'engines'
                    ? 'bg-white shadow-sm text-[#ED1C24]'
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-2 hide-scrollbar pb-2">
                  ...
                  {users.map((u: any) => {
                    ...
                    return (
                      <div
                        key={u.id}
                        className="flex flex-col bg-white p-3 rounded-xl border border-slate-100 shadow-sm shrink-0 mb-2"
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-3">
                            <span className="text-2xl opacity-90">
                              {getVehicleData(u.id).emoji}
                            </span>
                            <div>
                              <span className="block text-xs font-black text-slate-700 leading-tight">
                                {u.name}
                              </span>
                              <span
                          ...
                          <div className="flex gap-1.5">
                            <div
                              className={`p-1.5 rounded-full shadow-inner ${
                                isPrev
                                  ? 'bg-green-100 text-green-600'
                                  : 'bg-red-100 text-red-500'
                              }`}
                              title={`Preventiva: ${isPrev ? 'SÍ' : 'NO'}`}
                            >
                              <PhoneCall size={10} />
                            </div>
                            <div
                        ...
                        <div className="flex bg-slate-100 rounded-lg overflow-hidden border border-slate-200">
                          ...
                          <button
                            onClick={() => changeStatus(u.id, 'pits')}
                            className={`flex-1 py-1.5 text-[9px] font-black uppercase transition-colors border-l border-r border-slate-200 ${
                              u.status === 'pits'
                          ...
                          <button
                            onClick={() => changeStatus(u.id, 'penalty')}
                            className={`flex-1 py-1.5 text-[9px] font-black uppercase transition-colors ${
                              u.status === 'penalty'
                ...
                <div className="mt-2 pt-3 border-t border-slate-200 shrink-0">
                  {!confirmReset ? (
                    <button
                      ...
                      className="w-full py-2.5 bg-yellow-100 text-yellow-800 font-black rounded-xl transition-colors border border-yellow-300 text-xs flex justify-center items-center gap-2 active:scale-95"
                    >
                      <RefreshCw size={14} /> Iniciar Nuevo Día (Reset
                      Preventiva)
                    </button>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2.5 bg-red-600 text-white font-black rounded-xl transition-transform active:scale-95 text-[10px] shadow-md uppercase tracking-wider"
                      >
                        Confirmar Reset
                      </button>
                      <button
                        onClick={() => setConfirmReset(false)}
                        className="flex-1 py-2.5 bg-slate-200 text-slate-700 font-black rounded-xl transition-transform active:scale-95 text-[10px] uppercase tracking-wider"
                      >
                        Cancelar
                      </button>
                    </div>
              ...
              <div className="w-full flex-1 flex flex-col min-h-0">
                <div className="mb-4 shrink-0 bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={
                        !newUserName.trim() || users.length >= MAX_USERS
                      }
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-2 hide-scrollbar pb-4 min-h-0">
                  {users.map((u: any) => (
                    <div
                      key={u.id}
                      className="flex items-center justify-between bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm shrink-0 mb-2"
                    >
                        ...
                        {confirmDeleteId === u.id ? (
                          <>
                            <button
                              ...
                              className="bg-red-500 text-white text-[9px] font-black uppercase px-2 py-1.5 rounded-md active:scale-95 transition-transform"
                            >
                              Borrar
                            </button>
                            <button
                              onClick={() => setConfirmDeleteId(null)}
                              className="bg-slate-200 text-slate-600 p-1.5 rounded-md"
                            >
                              <X size={12} />
                            </button>
                          </>
                          ...
                          <>
                            <button
                              ...
                              className="text-slate-400 hover:text-blue-500 p-1.5 bg-slate-50 rounded-md transition-colors"
                            >
                              <Edit2 size={12} />
                            </button>
                            <button
                              ...
                              className="text-slate-400 hover:text-red-500 p-1.5 bg-slate-50 rounded-md transition-colors disabled:opacity-30"
                            >
                              <Trash2 size={12} />
                            </button>
                          </>
            ...
            <button
              onClick={onClose}
              className="w-full mt-2 py-3 bg-slate-800 text-white font-black rounded-2xl active:scale-95 transition-transform shrink-0 shadow-[0_10px_20px_rgba(0,0,0,0.15)]"
            >
              Cerrar Panel
            </button>
          </div>
...
export default function App() {
  ...
  const updateUserKpis = useCallback(
    async (id: any, newKpis: any, preventiva: any) => {
      try {
  ...
  const resetDay = useCallback(async () => {
    try {
      ...
      console.error('Error reiniciando día:', e);
    }
  }, [users]);
  ...
  return (
    <div className="font-['Nunito',sans-serif] bg-[#F8FAFC] min-h-screen text-slate-800 selection:bg-red-200">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div
          className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-red-200/40 rounded-full blur-3xl opacity-50 animate-blob"
          style={{ transform: 'translateZ(0)' }}
        />
        <div
          className="absolute top-[20%] right-[-10%] w-[50%] h-[50%] bg-slate-300/40 rounded-full blur-3xl opacity-50 animate-blob animation-delay-2000"
          style={{ transform: 'translateZ(0)' }}
        />
      </div>
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center w-12 transition-all duration-300 ${
                  isActive
                    ? 'text-[#ED1C24] -translate-y-2'
                    : 'text-slate-400 hover:text-slate-600'
                }`}
              >
                <div
                  className={`absolute inset-0 bg-red-50 rounded-full transition-transform duration-400 ease-out ${
                ...
                />
                <span
                  className={`text-[9px] font-black uppercase tracking-wider transition-opacity duration-300 ${
